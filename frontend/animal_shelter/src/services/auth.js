...
import { get_user_id, get_user_type, save_user_id, set_user_type } from "./cache"
...
class Auth_Service {
    ...
    async handle_signup(username, password, email, shelter_address, shelter_category) {
        const user_type = get_user_type();
        const payload = { username, password, email, user_type, shelter_address, shelter_category };
    ...
    async handle_verification(code) {
        try {
            const response = await axios.post(base_url + "/verify-code", { ver_code: code });
